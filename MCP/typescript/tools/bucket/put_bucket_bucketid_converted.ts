/**
 * MCP Server function for Update properties of a bucket
 */
...
function getPut_Bucket_Bucket_IdHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.allowed_mime_types !== undefined) {
        queryParams.push(`allowed_mime_types=${args.allowed_mime_types}`);
            ...
            const url = `${config.baseUrl}/api/v2/put_bucket_bucket_id${queryString}`;
...
function createPut_Bucket_Bucket_IdTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "put_bucket_bucket_id",
            description: "Update properties of a bucket",
            parameters: {
        ...
        handler: getPut_Bucket_Bucket_IdHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getPut_Bucket_Bucket_IdHandler,
    createPut_Bucket_Bucket_IdTool
};