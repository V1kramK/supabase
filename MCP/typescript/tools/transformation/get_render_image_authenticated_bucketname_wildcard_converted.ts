/**
 * MCP Server function for Render an authenticated image with the given transformations
 */
...
function getGet_Render_Image_Authenticated_Bucket_Name_WildcardHandler(config: APIConfig): (ctx: any, request: MCPRequest) => Promise<MCPToolResult> {
    ...
    if (args.quality !== undefined) {
        queryParams.push(`quality=${args.quality}`);
            ...
            const url = `${config.baseUrl}/api/v2/get_render_image_authenticated_bucket_name_wildcard${queryString}`;
...
function createGet_Render_Image_Authenticated_Bucket_Name_WildcardTool(config: APIConfig): Tool {
    return {
        definition: {
            name: "get_render_image_authenticated_bucket_name_wildcard",
            description: "Render an authenticated image with the given transformations",
            parameters: {
        ...
        *: { type: "string", required: true, description: "" },
        ...
        handler: getGet_Render_Image_Authenticated_Bucket_Name_WildcardHandler(config)
    };
}
...
export {
    ...
    Tool,
    ToolDefinition,
    getGet_Render_Image_Authenticated_Bucket_Name_WildcardHandler,
    createGet_Render_Image_Authenticated_Bucket_Name_WildcardTool
};