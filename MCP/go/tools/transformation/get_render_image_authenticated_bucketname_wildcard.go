...
func Get_render_image_authenticated_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/render/image/authenticated/%s/%s%s", cfg.BaseURL, bucketName, *, queryString)
...
func CreateGet_render_image_authenticated_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_render_image_authenticated_bucketName_wildcard",
		mcp.WithDescription("Render an authenticated image with the given transformations"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_render_image_authenticated_bucketname_wildcardHandler(cfg),
	}
}
