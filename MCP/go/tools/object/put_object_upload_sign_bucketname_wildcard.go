...
func Put_object_upload_sign_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/object/upload/sign/%s/%s%s", cfg.BaseURL, bucketName, *, queryString)
...
func CreatePut_object_upload_sign_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_object_upload_sign_bucketName_wildcard",
		mcp.WithDescription("Uploads an object via a presigned URL"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_object_upload_sign_bucketname_wildcardHandler(cfg),
	}
}
