...
func Put_object_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/object/%s/%s", cfg.BaseURL, bucketName, *)
...
func CreatePut_object_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_object_bucketName_wildcard",
		mcp.WithDescription("Update the object at an existing key"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Put_object_bucketname_wildcardHandler(cfg),
	}
}
