...
func Post_object_upload_sign_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/object/upload/sign/%s/%s", cfg.BaseURL, bucketName, *)
...
func CreatePost_object_upload_sign_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_object_upload_sign_bucketName_wildcard",
		mcp.WithDescription("Generate a presigned url to upload an object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Post_object_upload_sign_bucketname_wildcardHandler(cfg),
	}
}
