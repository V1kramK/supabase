...
func Get_object_info_public_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/object/info/public/%s/%s", cfg.BaseURL, bucketName, *)
...
func CreateGet_object_info_public_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_object_info_public_bucketName_wildcard",
		mcp.WithDescription("Get object info"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_object_info_public_bucketname_wildcardHandler(cfg),
	}
}
