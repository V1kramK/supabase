...
func Post_object_sign_bucketname_wildcardHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		*, ok := *Val.(string)
		...
		url := fmt.Sprintf("%s/object/sign/%s/%s", cfg.BaseURL, bucketName, *)
...
func CreatePost_object_sign_bucketname_wildcardTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_object_sign_bucketName_wildcard",
		mcp.WithDescription("Generate a presigned url to retrieve an object"),
		...
		mcp.WithNumber("expiresIn", mcp.Required(), mcp.Description("")),
		mcp.WithObject("transform", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_object_sign_bucketname_wildcardHandler(cfg),
	}
}
