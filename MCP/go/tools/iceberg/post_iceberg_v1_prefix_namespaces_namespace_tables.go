...
func Post_iceberg_v1_prefix_namespaces_namespace_tablesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/iceberg/v1/%s/namespaces/%s/tables", cfg.BaseURL, prefix, namespace)
...
func CreatePost_iceberg_v1_prefix_namespaces_namespace_tablesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_iceberg_v1_prefix_namespaces_namespace_tables",
		mcp.WithDescription("Create a table in the given namespace"),
		...
		mcp.WithString("schema", mcp.Required(), mcp.Description("")),
		mcp.WithObject("spec", mcp.Description("")),
		mcp.WithBoolean("stage-create", mcp.Description("")),
		mcp.WithObject("write-order", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Post_iceberg_v1_prefix_namespaces_namespace_tablesHandler(cfg),
	}
}
