...
func Delete_iceberg_v1_prefix_namespaces_namespace_tables_tableHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["purgeRequested"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("purgeRequested=%v", val))
		}
		queryString := ""
		if len(queryParams) > 0 {
			queryString = "?" + strings.Join(queryParams, "&")
		}
		url := fmt.Sprintf("%s/iceberg/v1/%s/namespaces/%s/tables/%s%s", cfg.BaseURL, prefix, namespace, table, queryString)
...
func CreateDelete_iceberg_v1_prefix_namespaces_namespace_tables_tableTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_iceberg_v1_prefix_namespaces_namespace_tables_table",
		mcp.WithDescription("Drop a Table"),
		mcp.WithString("purgeRequested", mcp.Description("If true, the table will be permanently deleted")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Delete_iceberg_v1_prefix_namespaces_namespace_tables_tableHandler(cfg),
	}
}
